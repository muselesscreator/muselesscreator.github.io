/* copied from wheel-of-prizes npm package */
...
const WheelComponent = ({
  ...
  let timerHandle = 0 as number | NodeJS.Timeout;
  ...
  let targetSegment = winningSegment;
  useEffect(() => {
    wheelInit();
    setTimeout(() => {
      window.scrollTo(0, 1);
  ...
  const initCanvas = () => {
    let canvas = document.getElementById(canvasId.current) as HTMLCanvasElement;
  ...
  const spin = () => {
    ...
    console.log("spin");
    if (timerHandle === 0) {
      console.log({ newWinner: targetSegment });
      onSpin && onSpin();
      targetSegment = `${Math.floor(Math.random() * segments.length)}`;
  ...
  const onTimerTick = () => {
    ...
    if (duration < upTime) {
      ...
      if (targetSegment) {
        console.log({ targetSegment, currentSegment });
        if (currentSegment === targetSegment && frames > segments.length) {
          progress = duration / upTime;
          angleDelta =
  ...
  const drawSegment = (key: number, lastAngle: number, angle: number) => {
    ...
    const newAngle = (lastAngle + angle) / 2;
    ctx.rotate(newAngle);
